...
public class JdbcPizzaRepositoryTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void findAll() {
		...
		pizzas.stream().map(pizza -> pizza.getId())
		.reduce((vorigeId, id) -> {assertTrue(id > vorigeId); return id;});
	}
	...
	@Test
	public void findUniekPrijzenGeeftPrijzenOplopend() {
		...
		prijzen.stream().reduce((vorigePrijs, prijs) -> {assertTrue(prijs.compareTo(vorigePrijs) > 0);
		return prijs;});
	}
